...
% Invasive - first calculate row sum of G, then create combinations
if ~iscell(G)
    A =[0;1];
if couplingtype == 1
...
% Non Invasive create all combination 
...
function y=rowsum(bl);
    [nn]=size(bl);
    bl2=bl*ones(nn(2),1); % sum of each row
    bl2=abs(bl2-bl2(1)*ones(nn(1),1)); % compare with the sum of the first row
   if max(bl2)==0,
       y=1;
   else
       y=0;
